...
class HomeViewModel: ObservableObject {
    ...
    @Published var createdMatch: Match?          // ← добавлено
        ...
        loadTask = Task {
            ...
            do {
                ...
                for i in loadedPets.indices {
                    loadedPets[i].matchPercentage = Int.random(in: 60...99)
                }
                pets = loadedPets
            } catch {
    ...
    func handleSwipe(pet: Pet, liked: Bool) {
        Task {
            do {
                ...
                if pets.count < 3 { await loadFeed() }
            } catch {
    ...
    func skipCurrent() {
        guard let first = pets.first else { return }
